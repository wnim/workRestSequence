...
import { BlocksBaseline } from '../../components/timeline/BlocksBaseline';
...
describe('BlocksBaseline', () => {
  it('renders an SVG element', () => {
    const { container } = render(<BlocksBaseline blocks={[workBlock]} pxPerSecond={20} width={300} />);
    expect(container.querySelector('svg')).toBeInTheDocument();
  });

  it('renders no path element when blocks is empty', () => {
    const { container } = render(<BlocksBaseline blocks={[]} pxPerSecond={20} width={300} />);
  ...
  it('SVG width matches the width prop', () => {
    const { container } = render(<BlocksBaseline blocks={[workBlock]} pxPerSecond={20} width={500} />);
    expect(container.querySelector('svg').getAttribute('width')).toBe('500');
  });

  it('path starts at CENTER_Y for any block type', () => {
    const { container: c1 } = render(<BlocksBaseline blocks={[workBlock]} pxPerSecond={20} width={300} />);
    const { container: c2 } = render(<BlocksBaseline blocks={[restBlock]} pxPerSecond={20} width={300} />);
  ...
  it('path spans the combined duration of multiple blocks', () => {
    const { container } = render(
      <BlocksBaseline blocks={[workBlock, restBlock]} pxPerSecond={20} width={400} />
    );
    const d = container.querySelector('path').getAttribute('d');
